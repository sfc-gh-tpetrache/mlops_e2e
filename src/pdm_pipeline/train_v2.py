"""Pipeline step 2b: train V2 = XGBoost (candidate).

Standalone ML Job or DAG task. Same interface + signature as train_v1 so the
two versions are directly A/B comparable behind the gateway.
"""
import argparse
import json
...
def main(env: str = "DEV", dataset_name: str | None = None) -> str:
    ...
    # Experiment Tracking: log this run's params + metrics (with XGBoost callback)
    ...
    model = xgb.XGBClassifier(
        ...
        early_stopping_rounds=2, n_estimators=10,
        callbacks=[callback],
    )
    with exp.start_run(run_name="XGBoost_V2"):
        exp.log_param("algorithm", "XGBoost")
        exp.log_param("n_estimators", 10)
        model.fit(
            train_df[features], train_df[common.TARGET],
            eval_set=[(test_df[features], test_df[common.TARGET])], verbose=0,
        )
    ...
    mv = reg.log_model(
        ...
        version_name=common.VERSION_V2,
        metrics=metrics,
        comment="XGBoost candidate (V2) to predict machine failures",
    ...
    print(json.dumps({"version": mv.version_name, "algorithm": "XGBoost", "metrics": metrics}))
